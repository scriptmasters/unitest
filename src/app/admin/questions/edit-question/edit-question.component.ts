  ...
  getAnswersOfSelectedQuestion(question_id) {
    this.questionService.getAnswersByQuestionId(question_id).subscribe(data => {
      if ( data.length ) {
        ...
        console.log('from getAnswersOfSelectedQuestion: this.editedAnswersArray = ', this.editedAnswersArray);
...
// set the array of id of received answers (if they exist) of the selected question
 setAnswersIdNumbersArray() {
  ...
  console.log('Завдання ще немає відповідей this.answersIdNumbersArray = ', this.answersIdNumbersArray);
  } else {
  ...
  console.log('from setAnswersIdNumbersArray: this.answersIdNumbersArray = ', this.answersIdNumbersArray);
 }
 ...
 addAnswer() {
  if (this.correctAnswerInputType === 'num' ) {
            ...
            if (this.editedAnswersArray.length === 0) { // creates two new answers
              this.insertNewAnswerToDataBase(this.sel_question_id, '', '1', '');
              this.insertNewAnswerToDataBase(this.sel_question_id, '', '1', '');
              this.getAnswersOfSelectedQuestion(this.sel_question_id);
            }

            if (this.editedAnswersArray.length === 1) { // one answer is edited and second is created
              this.insertNewAnswerToDataBase(this.sel_question_id, '', '1', '');
              this.getAnswersOfSelectedQuestion(this.sel_question_id);
            }

            if (this.editedAnswersArray.length >= 2) { // 1-st and 2-nd answers ere edited and rest is deleted
    ...
    this.questionForm.addControl( (this.editedAnswersArray.length + 1).toString(),  new FormControl('', [Validators.required] ) );
  ...
  setCorrectAnswerInputType() {
    if ( this.sel_question_type === '1') { this.correctAnswerInputType = 'radio'; }
    if ( this.sel_question_type === '2') { this.correctAnswerInputType = 'checkbox'; }
    if ( this.sel_question_type === '3') { this.correctAnswerInputType = 'txt'; }
    if ( this.sel_question_type === '4') { this.correctAnswerInputType = 'num'; }
    console.log('from setCorrectAnswerInputType: correctAnswerInputType = ', this.correctAnswerInputType);
  }
  ...
  onAnswerTypeSelect(event) {
    ...
    console.log('from onAnswerTypeSelect: correctAnswerInputType = ', this.correctAnswerInputType);
    console.log('from onAnswerTypeSelect: this.editedAnswersArray = ',  this.editedAnswersArray);
  }
  ...
  setAnswersResourse() {
    ...
    const haveEveryAnswerEmptyAttachment = this.editedAnswersArray.every(element => element.attachment === '');
    console.log('from setAnswersResourse: haveEveryAnswerEmptyAttachment = ', haveEveryAnswerEmptyAttachment);
    const hasSomeAnswerAttachment = this.editedAnswersArray.some(element => element.attachment === '');
    const haveEveryAnswerEmptyText = this.editedAnswersArray.every(element => element.answer_text === '');

    if ( haveEveryAnswerEmptyAttachment ) { this.answersResourse = 'text'; }
    if ( (hasSomeAnswerAttachment) && (haveEveryAnswerEmptyText) ) { this.answersResourse = 'file'; }
    if ( hasSomeAnswerAttachment ) { this.answersResourse = 'text_file'; }
    console.log('hasSomeAnswerAttachment = ', hasSomeAnswerAttachment);
    console.log('from setAnswersResourse: answersResourse = ', this.answersResourse);
  }

  onAnswerResourseSelect(event) {
    if (event.target.value === '1') {
       this.answersResourse = 'text';
       this.editedAnswersArray.forEach(element => { element.attachment = ''; }); // clears all answers attachments
       }
    if (event.target.value === '2') {
       this.answersResourse = 'file';
       this.editedAnswersArray.forEach(element => { element.answer_text = ''; }); // clears all answers text
       }
    if (event.target.value === '3') { this.answersResourse = 'text_file'; }
  ...
  setAnsverAttachment(checkedIndex, event) {
    const fileReader = new FileReader();
    const img = event.target.files[0];
    fileReader.onload = () => this.editedAnswersArray[checkedIndex].attachment = fileReader.result;
    fileReader.readAsDataURL(img);
    console.log('this.editedAnswersArray = ', this.editedAnswersArray);
  }
  ...
  setAnsverText(checkedIndex, event) {
    ...
    console.log('this.editedAnswersArray = ', this.editedAnswersArray);
    console.log(`editedAnswersArray[${checkedIndex}].answer_text = `, this.editedAnswersArray[checkedIndex].answer_text);
  }
  ...
  setAnswerCorrect(checkedIndex, event) {
      if (this.correctAnswerInputType === 'radio') {
        for (let index = 0; index < this.editedAnswersArray.length; index++) {
          this.editedAnswersArray[index].true_answer = (index === checkedIndex) ? '1' : '0';
        }
      }
  ...
  insertNewAnswerToDataBase(questionId, answerText = '', trueAnswer = '', answerAttachment = '') {
      const newAnswerJSON = JSON.stringify({
          ...
          this.questionService.addAnswer(newAnswerJSON).subscribe(
            (dataNewAnswer: IAnswersGet) =>
  ...
  deleteAnswer(checkedIndex) {
    this.deleteAnswerFromDataBase(this.editedAnswersArray[checkedIndex].answer_id);
    this.editedAnswersArray.splice(checkedIndex, 1);
    this.answersIdNumbersArray.splice(checkedIndex, 1);
    console.log(`from deleteAnswer: this.answersIdNumbersArray = `, this.answersIdNumbersArray);
    console.log(`from deleteAnswer: this.editedAnswersArray = `, this.editedAnswersArray);
  }
  ...
  const dialogRef = this.dialog.open(DeleteConfirmComponent, {
    ...
    dialogRef.afterClosed().subscribe((Response: boolean) => {
      if (Response) {
        this.questionService.deleteAnswer(checkedAnswerId).subscribe((data: IResponse) => {
          if (data.response === 'ok') {
