...
export class GroupsComponent extends Pagination implements OnInit, OnDestroy {
  ...
  searchFilterPipe = '';
  facultyFilterPipe = 'Виберіть факультет';
  specialityFilterPipe = 'Виберіть спецільність';
  pushTriger = true;
  sortBy = 'asc';

  refreshFilters() {
    this.searchFilterPipe = '';
    this.facultyFilterPipe = 'Виберіть факультет';
    this.specialityFilterPipe = 'Виберіть спецільність';
  }
    ...
    if (this.pushTriger) {
      ...
      this.groups.forEach((elem, i) => {
        ...
        for (const faculty of this.faculties) {
          if (elem.faculty_id === faculty.faculty_id) {
        ...
        for (const speciality of this.specialities) {
          if (elem.speciality_id === speciality.speciality_id) {
  ...
  sorting(prop) {
    if (this.sortBy === 'asc') {
      this.sortBy = 'desc';
      this.table.sort((a, b) => a[`${prop}`].localeCompare(b[`${prop}`]));
    } else if (this.sortBy === 'desc') {
      this.sortBy = 'asc';
      this.table.sort((a, b) => b[`${prop}`].localeCompare(a[`${prop}`]));
    }
  }
